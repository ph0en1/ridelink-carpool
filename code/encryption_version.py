...
def decrypt_credentials():
    # Provide the manual key directly here
    manual_key = b'pBwTYe7CxJVzD_d9ihyyRgK-RnA3yDHlJplwPGMB3mI=' 
    fernet = Fernet(manual_key)
    with open('encrypted_credentials.json', 'rb') as enc_file:
...
def emailSend(u_id, contact):
    ...
    message["Subject"] = "Carpool Request"
    message.set_content(f"Hello! I am an anonymous user contacting you through RideLink. I'm interested in arranging a carpool, you can contact me at: {contact}")
...
def emailWindow():
    ...
    tk.Label(window2, text="Enter User ID to send carpool request via email!", font=("Roboto", 23), bg="lightgrey", fg="black").pack(pady=17)
    userid_entry = tk.Entry(window2, font=("Roboto", 23), bg="lightgrey", fg="black")
    userid_entry.pack(pady=5)

    tk.Label(window2, text="Where can the receiver of your request contact you?", font=("Roboto", 23), bg="lightgrey", fg="black").pack(pady=20)
    contact_info = tk.Entry(window2, font=("Roboto", 23), bg="lightgrey", fg="black")
    ...
    tk.Button(window2, text="Send Email", font=("Times New Roman", 24), bg="deep sky blue", fg="white", command=handle_email_send).pack(pady=20)
...
def logintest():
    ...
    if login != "splendid":
        ...
        title = tk.Label(window, text="Carpool Finder", font=("Roboto", 40), bg="lightgrey", fg="black")
        title.pack(pady=10)

        tk.Label(window, text="Where is your house?", font=("Roboto", 20), bg="lightgrey", fg="black").pack(pady=40)

        global answer1
        answer1 = tk.Entry(window, width=15, font=("Times New Roman", 24), bg="lightgrey", fg="red")
        answer1.pack(pady=20)

        tk.Button(window, text="Set up Data", font=("Times New Roman", 24), bg="deep sky blue", fg="white", command=pre_cache).pack(pady=20)
        tk.Button(window, text="Find Nearby Addresses", font=("Times New Roman", 24), bg="deep sky blue", fg="white", command=mapFunc).pack(pady=20)
        tk.Button(window, text="Send Email", font=("Times New Roman", 24), bg="deep sky blue", fg="white", command=emailWindow).pack(pady=20)

tk.Label(loginWindow, text="Login Below!", font=("Roboto", 45), bg="lightgrey", fg="black").pack(pady=40)
tk.Label(loginWindow, text="Student Name", font=("Roboto", 23), bg="lightgrey", fg="black").pack(pady=25)
userName = tk.Entry(loginWindow, font=("Roboto", 23), bg="lightgrey", fg="black")
userName.pack(pady=10)

tk.Label(loginWindow, text="Student ID", font=("Roboto", 23), bg="lightgrey", fg="black").pack(pady=25)
userid = tk.Entry(loginWindow, font=("Roboto", 23), bg="lightgrey", fg="black")
userid.pack(pady=10)

tk.Button(loginWindow, text="Login", font=("Roboto", 23), bg="deep sky blue", fg="white", command=logintest).pack(pady=20)
